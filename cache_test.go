...
var _ = Describe("ObjectCodec", func() {
	...
	testCodec := func() {
		...
		It("Gets and Sets data", func() {
			...
			wanted, err := codec.GetObject(key)
			Expect(err).NotTo(HaveOccurred())
			wantedObj := wanted.(*Object)
			Expect(*wantedObj).To(Equal(*obj))
		...
		Describe("Once func", func() {
			It("calls Func when cache fails", func() {
				// Since object is sent directly into local cache,
				// no marshalling happens.
				// Thus GetObject/ObjectOnce will not return an error.
			})

			It("does not cache when Func fails", func() {
				perform(100, func(int) {
					gotPtr, err := codec.ObjectOnce(&cache.Item{
						Key:    key,
						Object: nil,
						Func: func() (interface{}, error) {
							return nil, io.EOF
					...
					Expect(err).To(Equal(io.EOF))
					Expect(gotPtr).To(BeNil())
				})

				_, err := codec.GetObject(key)
				Expect(err).To(Equal(cache.ErrCacheMiss))

				got, err := codec.ObjectOnce(&cache.Item{
					Key:    key,
					Object: 1,
					Func: func() (interface{}, error) {
						return true, nil
			...
			It("works with Object", func() {
				var callCount int64
				perform(100, func(int) {
					gotObj, err := codec.ObjectOnce(&cache.Item{
						Key: key,
						Func: func() (interface{}, error) {
							atomic.AddInt64(&callCount, 1)
							return obj, nil
					...
					gotPtr := gotObj.(*Object)
					Expect(err).NotTo(HaveOccurred())
					Expect(*gotPtr).To(Equal(*obj))
				})
				Expect(callCount).To(Equal(int64(1)))
			})
			...
			It("works without Object and nil result", func() {
				var callCount int64
				perform(100, func(int) {
					_, err := codec.ObjectOnce(&cache.Item{
						Key: key,
						Func: func() (interface{}, error) {
							atomic.AddInt64(&callCount, 1)
							return nil, nil
			...
			It("does not cache error result", func() {
				var callCount int64
				do := func(sleep time.Duration) (interface{}, error) {
					nObj, err := codec.ObjectOnce(&cache.Item{
						Key: key,
						Func: func() (interface{}, error) {
							time.Sleep(sleep)
	...
	Context("with LocalObjectCache", func() {
		BeforeEach(func() {
			codec = newCodec()
			codec.UseLocalObjectCache(1000, time.Minute)
		})

		testCodec()
	})

	Context("with LocalObjectCache and without Redis", func() {
		BeforeEach(func() {
			codec = newCodec()
			codec.UseLocalObjectCache(1000, time.Minute)
			codec.Redis = nil
